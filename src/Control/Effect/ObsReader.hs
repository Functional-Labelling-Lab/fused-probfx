...
-- | The effect @ObsReader@ for reading observed values from a model environment @env@
data ObsReader (env :: [Assign Symbol *]) (m :: Type -> Type) (k :: Type) where
  ...
  Ask :: Observable env x a
    ...
    -> ObsReader env m (Maybe a)

-- | Smart Constructor for the 'ObsReader' effect
ask :: forall env sig m x a. (Has (ObsReader env) sig m, Observable env x a)
  => ObsVar x    -- ^ variable @x@ to read from
  -> m (Maybe a) -- ^ the head value from @x@'s list
