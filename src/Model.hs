...
newtype Model env sig m a =
  Model { runModel :: (Has (Dist a) sig m, Has (ObsReader env) sig m) => m a }
  deriving Functor
-- newtype Model env sig m a =
...
handleCore :: (Has (Observe a) sig m , Has (Sample a) sig m) => Model env (ObsReader env :+: sig) m a -> Env env -> Prog es a
