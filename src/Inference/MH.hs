  ...
  alg hdl sig ctx = SampObsC $ case sig of
    L (SampObs (PrimDistPrf d) obs addr) -> case obs of
      ...
      Nothing -> do
        ...
        x <- lookupSample strace d addr α_samp
        pure $ x <$ ctx
...
mh :: (FromSTrace env, Has Sampler sig m)
  => Int -- number of MH samplings
  -> ObsReaderC env (DistC (SampTracerC (LPTracerC (SampObsC m)))) a -- ^ model awaiting an input
  ...
  -> m [Env env] -- ^ [output model environment]
...
mhRaw :: (FromSTrace env, Has Sampler sig m)
  => Int -- number of MH samplings
  -> ObsReaderC env (DistC (SampTracerC (LPTracerC (SampObsC m)))) a -- ^ model awaiting an input
  -> Env env        -- ^ (model input, input model environment)
  -> [Tag] -- ^ optional list of observable variable names (strings) to specify sample sites of interest
...
mhStep :: (Has Sampler sig m)
  => ObsReaderC env (DistC (SampTracerC (LPTracerC (SampObsC m)))) a -- ^ model
  -> Env env                  -- ^ model environment
  -> [Tag]                    -- ^ tags indicating sample sites of interest
  -> ((a, STrace), LPTrace) -- ^ trace of previous MH outputs
  -> m (Maybe ((a, STrace), LPTrace)) -- ^ updated trace of MH outputs
mhStep model env tags trace = do
  ...
  let acceptance_ratio = accept α_samp samples samples' logps logps'
...
runMH :: Has Sampler sig m
  => ObsReaderC env (DistC (SampTracerC (LPTracerC (SampObsC m)))) a -- ^ model
  -> Env env        -- ^ model environment
  -> STrace         -- ^ sample trace of previous MH iteration
  -> Addr           -- ^ sample address of interest
  -> m ((a, STrace), LPTrace) -- ^ (model output, sample trace, log-probability trace)
runMH model env strace α_samp =
  runReader (strace, α_samp) $ runSampObs $ runLPTracer True $ runSampTracer $ runDist $ runObsReader env model
...
lookupSample :: (OpenSum.Member a PrimVal, Has Sampler sig m)
  =>
     STrace     -- ^ sample trace
...
accept :: Addr -- ^ address of new sampled value
  ...
  -> Double
accept x0 _Ⲭ _Ⲭ' logℙ logℙ' = exp (dom_logα + _X'logα - _Xlogα)
  where
    _X'sampled = Set.singleton x0 `Set.union` (Map.keysSet _Ⲭ' \\ Map.keysSet _Ⲭ)
    _Xsampled  = Set.singleton x0 `Set.union` (Map.keysSet _Ⲭ \\ Map.keysSet _Ⲭ')
    dom_logα   = log (fromIntegral $ Map.size _Ⲭ) - log (fromIntegral $ Map.size _Ⲭ')
    _Xlogα     = foldl (\logα v -> logα + fromJust (Map.lookup v logℙ))
                         0 (Map.keysSet logℙ \\ _Xsampled)
    _X'logα    = foldl (\logα v -> logα + fromJust (Map.lookup v logℙ'))
